...
- (void)				stopScaleChange
{
   // do we need to manipulate self.zooming here?
	//mIsChangingScale = NO;
	[self setNeedsDisplay];	// redraw in high quality?
	
	LogEvent_( kReactiveEvent, @"view stopScaleChange called (%f): %@", [self scale], self );
}
*/
/*
- (void)				startScaleChange
...
///
///********************************************************************************************************************
/* no scroll wheel on a touch device!
...
@implementation UIView (DKTNSViewEmulation)

- (CGRect)visibleRect
{
   // this is a CAScrollLayer property on iPhone
   CALayer *backingLayer = self.layer;
   CGRect visibleRect = [backingLayer visibleRect];
   return visibleRect;
}
...
- (void)setNeedsDisplay:(BOOL)flag
{
   if (flag)
      [self setNeedsDisplay];
   // there is apparently no region validation on iPhone
}

- (void)getRectsBeingDrawn:(const NSRect **)rects count:(NSInteger *)count
{
   // there is not actually a way to access the invalid region of a UIView?
   static NSRect sCurrentDrawnRect;
   sCurrentDrawnRect = self.bounds;
   *rects = &sCurrentDrawnRect;
   *count = 1;
}
...
- (NSData *)dataWithPDFInsideRect:(CGRect)rect
{
   ...
   // some way to implmement this before 3.2 UIGraphics.h UIGraphicsBeginPDFContextToData() and friends?
   twlog("implement dataWithPDFInsideRect");
   return nil;
}
