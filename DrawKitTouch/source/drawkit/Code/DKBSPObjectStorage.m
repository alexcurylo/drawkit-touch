...
static inline NSUInteger depthForObjectCount( NSUInteger n )
{
  ...
  //return  ( n > 0? MAX((NSUInteger) _CGFloatCeil(_CGFloatLog((CGFloat) n)) / _CGFloatLog(2.0f), kDKMinimumDepth ) : 0 );
   if (1 > n)
      return 0;
   NSUInteger ceiling = _CGFloatCeil(_CGFloatLog((CGFloat) n)) / _CGFloatLog(2.0f);
   return MAX(ceiling, kDKMinimumDepth);
}
...
- (void)			setDepth:(NSUInteger) depth
{
   ...
   // we'll make this #if instead of if to avoid constant expression warning ...alex
