...
static UIColor*			sPageBreakColour = nil;
...
+ (UIColor*)			pageBreakColour
{
	if ( sPageBreakColour == nil )
	{
		...
		sPageBreakColour = [[[UIColor cyanColor] colorWithAlphaComponent:0.75] retain];
	}
...
+ (UIColor*)			backgroundColour
{
	...
	return [UIColor colorWithCalibratedRed:0.75 green:0.75 blue:0.8 alpha:1.0];
}
...
+ (Class)				classForTextEditor
{
	if( s_textEditorClass == nil )
		...
		s_textEditorClass = [UITextView class];
...
+ (void)				setClassForTextEditor:(Class) aClass
{
	...
	if([aClass isSubclassOfClass:[UITextView class]])
...
- (NSTextView*)			editText:(NSAttributedString*) text inRect:(NSRect) rect delegate:(id) del drawsBackground:(BOOL) drawBkGnd
{
	...
	[m_textEditViewRef setNeedsDisplay];
...
- (void)				endTextEditing
{
   twlog("implement endTextEditing");
   /*
	if ([self isTextBeingEdited])
	{
...
- (void)				moveRulerMarkerNamed:(NSString*) markerName toLocation:(CGFloat) loc
{
   (void)markerName;
   (void)loc;
   twlog("implement moveRulerMarkerNamed");
...
- (IBAction)			toggleRuler:(id) sender
{
   ...
   twlog("need to implement rulers!");
/*
...
- (void)				windowActiveStateChanged:(NSNotification*) note
{
   ...
   if (self.window.isKeyWindow)
      self.backgroundColor = [[self class] backgroundColour];
   else
...
///
///********************************************************************************************************************
/* regular resizing should take care of this?
...
// is this a correct place to substitute this check?
- (void)layoutSubviews
{
...
///
///********************************************************************************************************************
/* can we emulate this by UIApplication overriding sendAction: perhaps?
