...
#if TARGET_OS_IPHONE
   (void)rtfData;
   twlog("implement textShapeWithRTFData");
#else
...
#if TARGET_OS_IPHONE
   twlog("implement textShapeWithAttributedString");
   return nil;
#else
	...
	[te setAlignment:DKLeftTextAlignment];
	[te setText:str];
...
+ (DKBezierPath*)			textOverflowIndicatorPath
{
	//static NSBezierPath* mtp = nil;
	static DKBezierPath* mtp = nil;
	
	if( mtp == nil )
	{
		//mtp = [[NSBezierPath bezierPathWithRect:NSMakeRect( 0, 0, 1, 1 )] retain];
		mtp = [[DKBezierPath bezierPathWithRect:NSMakeRect( 0, 0, 1, 1 )] retain];
...
{
#if TARGET_OS_IPHONE
   return (nil != pb.string) && ![self locked];
#else
...
- (IBAction)				alignLeft:(id) sender
{
	...
	[self setAlignment:DKLeftTextAlignment];
}
...
- (IBAction)				alignRight:(id) sender
{
	...
	[self setAlignment:DKRightTextAlignment];
}
...
- (IBAction)				alignCenter:(id) sender
{
	...
	[self setAlignment:DKCenterTextAlignment];
}
...
- (IBAction)				verticalAlign:(id) sender
{
	...
	if ( ![self locked] )
	{
      ...
      // we'll remove casts to quiet our compiler warnings ...alex
		[self setVerticalAlignment:[sender tag]];
...
- (IBAction)				capitalize:(id) sender
{
	if( ![self locked])
	{
      ...
      // we'll remove casts to quiet our compiler warnings ...alex
		[[self textAdornment] setCapitalization:[sender tag]];
...
- (id)					initWithStyle:(DKStyle*) aStyle
{
	...
	if (self != nil)
	{
		...
		[self setPath:[DKBezierPath bezierPathWithRect:[DKDrawableShape unitRectAtOrigin]]];
...
#if TARGET_OS_IPHONE
   twlog("need to worry about text shape bounds?");
#else
	if ( m_editorRef )
...
- (void)				drawSelectedState
{
	...
	if(![[self textAdornment] allTextWasFitted] && [[self class] showsTextOverflowIndicator])
	{
		...
		DKBezierPath* np = [[self class] textOverflowIndicatorPath];
...
#if TARGET_OS_IPHONE
   (void)pb;
   twlog("implement writeSupplementaryDataToPasteboard");
#else
...
- (BOOL)				validateMenuItem:(NSMenuItem*) item
{
	...
	if ( action == @selector( verticalAlign: ))
	{
      ...
      // we'll remove casts to quiet our compiler warnings ...alex
		[item setState:((NSUInteger)[item tag] == [self verticalAlignment])? NSOnState : NSOffState];
	...
	if ( action == @selector( capitalize: ))
	{
      ...
      // we'll remove casts to quiet our compiler warnings ...alex
		[item setState:[[self textAdornment] capitalization] == (NSUInteger)[item tag]? NSOnState : NSOffState];
