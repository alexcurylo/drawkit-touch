...
+ (DKHandle*)			handleForType:(DKKnobType) type size:(NSSize) size colour:(DKColor*) colour
{
   ...
   int cellWidth = ceil(size.width);
   int cellHeight = ceil(size.height);
	if( colour )
		key = [NSString stringWithFormat:@"%@_%@_%dx%d", classKey, [colour hexString], cellWidth, cellHeight];
	else
		key = [NSString stringWithFormat:@"%@_%dx%d", classKey, cellWidth, cellHeight];
