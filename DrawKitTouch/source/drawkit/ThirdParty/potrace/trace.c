...
static inline double dpara(dpoint_t p0, dpoint_t p1, dpoint_t p2) {
   //double x1, y1, x2, y2;
   // prefix added to avoid compiler shadow warning ...alex
...
static int adjust_vertices(privpath_t *pp) {
  ...
  for (i=0; i<m; i++) {
    ...
    dy = fabs(w.y-s.y);
    if (dx <= .5 && dy <= .5) {
      pp->curve.vertex[i].x = w.x+px0;
      pp->curve.vertex[i].y = w.y+py0;
      continue;
    }
  ...
  corners:
    ...
    pp->curve.vertex[i].x = xmin + px0;
    pp->curve.vertex[i].y = ymin + py0;
    continue;
  }
